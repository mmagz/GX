...
async function main() {
  ...
  // Check if capsule banner image exists
  const capsuleImage = 'capsule.JPG'
  const imagePath = path.join(imagesDir, capsuleImage)
  
  if (!fs.existsSync(imagePath)) {
    console.error('Capsule banner image not found at:', imagePath)
    process.exit(1)
  }
  ...
  console.log('Uploading capsule banner to Cloudinary...')
  
  try {
    // Upload the banner image
    const cloudFolder = 'xoned/banners'
    const bannerUrl = await uploadImage(imagePath, cloudFolder)
    console.log(`Uploaded ${capsuleImage} -> ${bannerUrl}`)

    // Write mapping to file
    const mappingOutput = {
      [capsuleImage]: bannerUrl,
      bannerUrl: bannerUrl,
    ...
    const mapPath = path.join(uploadsDir, 'capsule-banner-cloudinary-map.json')
    fs.writeFileSync(mapPath, JSON.stringify(mappingOutput, null, 2))

    console.log('Capsule banner upload complete.')
    console.log('Banner URL:', bannerUrl)
    console.log('Mapping written to:', mapPath)
    console.log('You can now use this URL in your frontend for the capsule banner!')

  } catch (error) {
    console.error('Error uploading capsule banner:', error)
  }
}
